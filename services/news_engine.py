 """services/news_engine.py - RSS Feed Parser for financial news."""
import feedparser
...
def fetch_ticker_news(ticker_or_topic: str, hours_back: int = 48) -> list[NewsItem]:
    cutoff = datetime.now(timezone.utc) - timedelta(hours=hours_back)
    results = []
    
    query_lower = ticker_or_topic.lower() if ticker_or_topic else ""
    ...
    for name, url in FEEDS.items():
        for item in fetch_feed(name, url):
            ...
            # Match by ticker OR match by general topic
            if not ticker_or_topic:
                results.append(item)
            elif ticker_or_topic.upper() in item.tickers:
                results.append(item)
            elif query_lower in full_text.lower():
                results.append(item)
